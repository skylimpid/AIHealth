...
class DecetorNet(object):
    ...
    def getDetectorNet(self, X, coord):
        ...
        # construct preblock
        preBlock_0 = tf.layers.conv3d(X, 24, kernel_size=(3, 3, 3), strides=(1, 1, 1), padding="same", data_format=self.DATA_FORMAT)
...
#        print(output_2.shape)
        ...
        out = tf.reshape(output_2, (size[0], size[1], -1))
        out = tf.transpose(out, perm=(0, 2, 1))
        out = tf.reshape(out, (size[0], size[2], size[3], size[4], len(config['anchors']), 5))
#        print (out.shape)
        return feat, out
